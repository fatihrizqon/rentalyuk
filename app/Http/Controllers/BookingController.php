...
class BookingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($from && $to){  
            ...
            $bookings = Booking::whereBetween('created_at', [$from, $to])->with(['user' => function($query){ return $query->select('id', 'username');},'vehicle' => function($query){ return $query->select('id', 'name');}])->orderBy('created_at', 'desc')->sortable()->paginate(10);              
        }elseif($keywords){
            $bookings = Booking::where('code', 'like', '%' . $keywords . '%')->with(['user' => function($query){ return $query->select('id', 'username');},'vehicle' => function($query){ return $query->select('id', 'name');}])->orderBy('created_at', 'desc')->sortable()->paginate(10);           
        }else{
            $bookings = Booking::orderBy('created_at', 'desc')->sortable()->with(['user' => function($query){ return $query->select('id', 'username');},'vehicle' => function($query){ return $query->select('id', 'name');}])->paginate(10);
        }
